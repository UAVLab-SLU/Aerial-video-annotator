...
class StreamingExample:
    ...
    def show_yuv_frame(self, yuv_frame):
        ...
        if data != None: # if NEW data has been received since last ReadReceivedData function call
            ...
            c = GC.CameraRayProjection(67,[float(data["lat"]),float(data["lon"]),float(data["alt"])],[691,337],GC.Coordinates(int(data["xpos"]),int(data["ypos"])),[float(data["x"]), float(data["y"]), float(data["z"]), float(data["w"])])
    ...
    def fly(self):
        ...
        self.drone(moveBy(0,0,-30,0,_timeout=100)).wait().success()
        self.drone(moveBy(50,0,0,0,_timeout=100)).wait().success()
        self.drone(moveBy(0,50,0,0,_timeout=100)).wait().success()
        self.drone(moveBy(-50,0,0,0,_timeout=100)).wait().success()
        self.drone(moveBy(0,-50,0,0,_timeout=100)).wait().success()
        self.drone(moveBy(0,0,30,0,_timeout=100)).wait().success()
        # for i in range(4):
